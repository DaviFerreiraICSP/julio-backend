...
router.post('/:id/confirm', (req, res) => {
  ...
  if (typeof confirmed !== 'boolean') {
    return res.status(400).json({ error: 'Campo confirmed (boolean) é obrigatório.' })
  }
  ...
  if (confirmed) {
    ...
    if (count > invite.max_guests) {
      return res.status(400).json({ error: `Máximo de ${invite.max_guests} convidado(s) para este convite.` })
    }
